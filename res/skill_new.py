...
class SkillNormalizer:
    ...
    def skill_nor(self, input_skill,value):
        #start=time.time()
        result = []
        input_skill = re.sub(r'[|,，、/]+', ' ', input_skill.strip())
        input_skill = re.sub(r'\s+', ' ', input_skill)
        ...
        seg_list=[]
        for y in input_skill.split(' '):
            seg_list.extend(jieba.lcut(y,cut_all=False))
        seg_list.extend(value.split(' '))
        """
        seg_list=[]
        ...
        for token in seg_list:
            result_=self.process_data(token,words_list)
            if result_:
                result.append(result_)
        #end=time.time()
        #print(seg_list,list(set(result)))
        #print('skill',end-start)
